...
export default function PixelGame() {
  ...
  useEffect(() => {
    ...
    const ground = H - 14;
    const px = 20;
    let py = ground, jumping = false, vy = 0;
    ...
    const loop = () => {
      ...
      if (jumping) {
        vy += 0.42;
        py += vy;
        if (py >= ground) { py = ground; jumping = false; vy = 0; }
      }

      obstacles.forEach(o => {
        o.x -= 2;
        if (o.x < -20) o.x = W + Math.random() * 100 + 60;
        ctx.fillStyle = '#ff6b00';
        ctx.fillRect(o.x, ground - o.h + 14, 6, o.h);
        ctx.fillStyle = '#cc4400';
        ctx.fillRect(o.x, ground - o.h + 14, 6, 2);
      });
