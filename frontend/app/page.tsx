...
export default function HomePage() {
  ...
  {FEATURES.map(({ icon, title, desc, color }) => (
    <div
      ...
      style={{
        background: 'rgba(140, 100, 210, 0.2)', // light translucent purple
        border: '1px solid rgba(140, 100, 210, 0.25)', // slightly stronger border
      }}
    >
      <div
        className="w-11 h-11 rounded-lg flex items-center justify-center mb-5 transition-transform group-hover:scale-110"
        style={{ background: `${color}33`, color: 'black' }} // slightly darker icon bg, black icon
      >
        {icon}
      </div>
